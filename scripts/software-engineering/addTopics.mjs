...
export function addTopics(additions) {
  ...
  const writeRecursive = (item, parentId, parentLevel, parentTags) => {
    const fullId = parentId ? `${parentId}--${item.id}` : item.id
    if (byId.has(fullId)) {
      console.log(`SKIP duplicate id: ${fullId}`)
      return
    }
    const tags = item.tags ?? parentTags ?? []
    const level = item.level ?? parentLevel ?? 'beginner'
    ...
    write(meta)
    ;(item.children ?? []).forEach((child) => writeRecursive(child, fullId, level, tags))
  }
  ...
  for (const item of additions) {
    ...
    write(meta)
    ;(item.children ?? []).forEach((child) => writeRecursive(child, item.id, level, tags))
  }
