...
const topics = readdirSync(TOPICS_DIR, { withFileTypes: true })
  ...
  .map((d) => ({
    dir: d.name,
    ...JSON.parse(readFileSync(resolve(TOPICS_DIR, d.name, 'topic.json'))),
  }))
...
const byTag = topics.reduce((acc, t) => {
  const tag = (t.tags ?? [])[0] ?? '(none)'
  acc[tag] = (acc[tag] ?? 0) + 1
  return acc
}, {})

console.log('--- Docker validation ---')
...
for (const stage of roadmap.stages) {
  ...
  console.log(`  ${stage.id.padEnd(28)} ${count}`)
}
